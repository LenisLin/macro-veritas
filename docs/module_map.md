...
| `src/macro_veritas/cli.py` | Lightweight `argparse` CLI scaffold plus the public `ingest study`, `ingest study --from-file`, `ingest dataset`, `ingest dataset --from-file`, `ingest claim`, `ingest claim --from-file`, `update dataset`, `show study`, `show dataset`, `show claim`, `list studies`, `list datasets`, `list claims`, `delete study`, `delete dataset`, and `delete claim` adapters | Keep existing public commands intact while exposing only the narrow StudyCard, DatasetCard, and ClaimCard ingest/create, single-file YAML ingest, DatasetCard full-replace update, show-by-id, family-level discovery, and referentially-aware by-id delete paths. |
...
| `src/macro_veritas/commands/update.py` | `update` family module with the real DatasetCard full-replace execution bridge beneath the public CLI adapter | Real DatasetCard update-input normalization, single-file YAML load for one complete canonical replacement card, CLI target/file match enforcement, plan/update gateway calls, and command result translation; StudyCard/ClaimCard/patch semantics remain deferred |
...
- The only public domain commands in this round are `ingest study`,
  ...
  `ingest claim`, `ingest claim --from-file`, `update dataset`,
...
- File input remains single-file YAML only at `ingest study --from-file`, `ingest dataset --from-file`, `ingest claim --from-file`, and `update dataset --dataset-id <ID> --from-file <path.yaml>`.
