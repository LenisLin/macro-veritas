...
| `src/macro_veritas/registry/layout.py` | Canonical first-slice relative paths plus StudyCard, DatasetCard, ClaimCard, internal history directories, and deterministic lock paths used by update and delete | Path/layout helper layer only; not a caller-facing IO API |
...
| `src/macro_veritas/registry/locks.py` | Internal advisory file-lock helper used by single-card update and delete execution | No distributed locking, no retries/timeouts, no lock registry or daemon |
...
- Single-card exclusive update/delete locking lives beneath that gateway
