...
- [`docs/public_ingest_studycard_cli.md`](public_ingest_studycard_cli.md), [`docs/public_ingest_datasetcard_cli.md`](public_ingest_datasetcard_cli.md), [`docs/public_ingest_claimcard_cli.md`](public_ingest_claimcard_cli.md), [`docs/public_show_cli.md`](public_show_cli.md), and [`docs/public_list_cli.md`](public_list_cli.md) define the current public domain command surfaces.
...
| `src/macro_veritas/cli.py` | Lightweight `argparse` CLI scaffold plus the public `ingest study`, `ingest dataset`, `ingest claim`, `show study`, `show dataset`, `show claim`, `list studies`, `list datasets`, and `list claims` adapters | Keep existing public commands intact while exposing only the narrow StudyCard, DatasetCard, and ClaimCard ingest/create, show-by-id, and family-level discovery paths. |
...
- The only public domain commands in this round are `ingest study`,
  ...
  `show claim`, `list studies`, `list datasets`, and `list claims`.
