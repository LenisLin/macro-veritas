...
- [`docs/delete_locking_policy.md`](delete_locking_policy.md) is the source of truth for single-card delete locking.
- [`docs/ingest_locking_policy.md`](ingest_locking_policy.md) is the source of truth for parent-aware DatasetCard ingest locking.
- [`docs/claim_ingest_locking_policy.md`](claim_ingest_locking_policy.md) is the source of truth for reference-aware ClaimCard ingest locking.
...
- [`docs/public_ingest_studycard_cli.md`](public_ingest_studycard_cli.md), [`docs/public_ingest_studycard_from_file.md`](public_ingest_studycard_from_file.md), [`docs/public_update_studycard_cli.md`](public_update_studycard_cli.md), [`docs/public_ingest_datasetcard_cli.md`](public_ingest_datasetcard_cli.md), [`docs/public_ingest_datasetcard_from_file.md`](public_ingest_datasetcard_from_file.md), [`docs/public_update_datasetcard_cli.md`](public_update_datasetcard_cli.md), [`docs/public_ingest_claimcard_cli.md`](public_ingest_claimcard_cli.md), [`docs/public_ingest_claimcard_from_file.md`](public_ingest_claimcard_from_file.md), [`docs/public_update_claimcard_cli.md`](public_update_claimcard_cli.md), [`docs/public_show_cli.md`](public_show_cli.md), [`docs/public_list_cli.md`](public_list_cli.md), and [`docs/public_delete_cli.md`](public_delete_cli.md) define the current public domain command surfaces.
...
| `src/macro_veritas/cli.py` | Lightweight `argparse` CLI scaffold plus the public `ingest study`, `ingest study --from-file`, `ingest dataset`, `ingest dataset --from-file`, `ingest claim`, `ingest claim --from-file`, `update study`, `update dataset`, `update claim`, `show study`, `show dataset`, `show claim`, `list studies`, `list datasets`, `list claims`, `delete study`, `delete dataset`, and `delete claim` adapters | Keep existing public commands intact while exposing only the narrow StudyCard, DatasetCard, and ClaimCard ingest/create, single-file YAML ingest, StudyCard, DatasetCard, and ClaimCard full-replace update, show-by-id, family-level discovery, and referentially-aware by-id delete paths. |
...
| `src/macro_veritas/registry/layout.py` | Canonical first-slice relative paths plus StudyCard, DatasetCard, ClaimCard, internal history directories, and deterministic lock paths used by DatasetCard ingest, ClaimCard ingest, update, and delete | Path/layout helper layer only; not a caller-facing IO API |
...
| `src/macro_veritas/registry/locks.py` | Internal advisory file-lock helper used by parent-aware DatasetCard ingest, reference-aware ClaimCard ingest, and single-card update/delete execution | No distributed locking, no retries/timeouts, no lock registry or daemon |
| `src/macro_veritas/registry/gateway.py` | Gateway contract descriptors plus the real StudyCard, DatasetCard, and ClaimCard runtime boundary | Real StudyCard, DatasetCard, and ClaimCard reads/listings/planning/create/update/delete with gateway-owned direct-reference and reverse-dependency checks plus parent-aware DatasetCard ingest locking and reference-aware ClaimCard ingest locking |
...
| `src/macro_veritas/registry/claim_runtime.py` | Internal ClaimCard-only runtime helper layer beneath the gateway | YAML serialization, minimal validation, canonical path checks, pre-update snapshot preservation, and single-card atomic writes only; direct StudyCard and DatasetCard existence checks stay in the gateway |
...
| `src/macro_veritas/commands/update.py` | `update` family module with the real StudyCard, DatasetCard, and ClaimCard full-replace execution bridges beneath the public CLI adapters | Real StudyCard, DatasetCard, and ClaimCard update-input normalization, single-file YAML load for one complete canonical replacement card, CLI target/file match enforcement, plan/update gateway calls, and command result translation; patch semantics remain deferred |
...
- The only public domain commands in this round are `ingest study`,
  ...
  `ingest claim`, `ingest claim --from-file`, `update study`,
  `update dataset`, `update claim`,
  `show study`, `show dataset`, `show claim`, `list studies`,
  `list datasets`, `list claims`, `delete study`, `delete dataset`,
  and `delete claim`.
- ClaimCard patch semantics remain out of scope.
...
- Update-time snapshot preservation remains beneath that gateway boundary in
  `macro_veritas.registry.history` and the family runtime helpers.
- Single-card exclusive update/delete locking plus parent-aware DatasetCard
  ingest locking and reference-aware ClaimCard ingest locking live beneath that gateway
  boundary in `macro_veritas.registry.locks`,
  `macro_veritas.registry.layout`, and the gateway mutation helpers.
