...
def test_registry_gateway_descriptors_match_frozen_boundary() -> None:
    ...
    assert error_semantics["UpdateLockError"]["applies_to"] == ("create", "update", "delete")
    ...
    assert atomic_policy["concurrent_locking"] == (
        "exclusive single-card update/delete lock plus parent-aware DatasetCard "
        "ingest locking plus reference-aware ClaimCard ingest locking"
    )
    assert "DatasetCard ingest" in atomic_policy["lock_scope"]
    assert "ClaimCard ingest" in atomic_policy["lock_scope"]
    assert "DatasetCard ingest" in atomic_policy["lock_lifetime"]
    assert "ClaimCard ingest" in atomic_policy["lock_lifetime"]
